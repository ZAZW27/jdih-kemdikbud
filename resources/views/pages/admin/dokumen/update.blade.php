    ...
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg">
        ...
        <div class="flex items-center justify-center h-auto py-2 px-4 rounded bg-gray-50">
            <div class="relative overflow-x-auto sm:rounded-lg py-4 w-full px-6">
                <form action="{{route('proses-update-dokumen')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="flex flex-col justify-center flex-wrap">
                        ...
                        <div class="relative z-0 w-full mb-4 group flex flex-wrap">
                            ...
                            <input id="status-dok" name="status" class="shadow ml-2 w-12 bg-white border border-slate-500 text-green-500 text-sm rounded-lg focus:ring-blue-300 focus:border-sky-800 block p-2.5 transition-all duration-200 ease-out" type="checkbox" id="email" aria-describedby="helper-text-explanation" @if($data->status_dok === 'Selesai') checked @endif>
                        </div>  
                    </div>
                    <div class="grid md:grid-cols-2 md:gap-6">
                        <div class="relative z-0 w-full mb-6 group">
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900">Peraturan</label>
                            <input value="{{$data->peraturan}}" placeholder="Jenis peraturan apa dokumen yang akan dibuat?" name="peraturan" type="text" id="email" aria-describedby="helper-text-explanation" class="shadow bg-white border border-slate-500 text-gray-900 text-sm rounded-lg focus:ring-blue-300 focus:border-sky-800 block w-full p-2.5  transition-all duration-200 ease-out" required>
                        </div>
                        <div class="relative z-0 w-full mb-6 group">
                            <label for="email" class="block mb-2 text-sm font-medium text-gray-900">OPD</label>
                            <input value="{{$data->OPD}}" placeholder="OPD. . ." name="opd" type="text" id="email" aria-describedby="helper-text-explanation" class="shadow bg-white border border-slate-500 text-gray-900 text-sm rounded-lg focus:ring-blue-300 focus:border-sky-800 block w-full p-2.5  transition-all duration-200 ease-out" required>
                        </div>
                    </div>
                    <div class="relative z-0 w-full mb-6 group">                        
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-900">keterangan</label>
                        <input value="{{$data->keterangan_dok}}" placeholder="keterangan dokumen " name="keterangan" type="text" id="email" aria-describedby="helper-text-explanation" class="shadow bg-white border border-slate-500 text-gray-900 text-sm rounded-lg focus:ring-blue-300 focus:border-sky-800 block w-full p-2.5  transition-all duration-200 ease-out" required>
                    </div>
